...
- **Non-fatal (accumulate and continue):** every per-repo operation. `syncRepo` wraps the repo fetch, label, settings and project work so failures are captured into `SyncErrorRecord[]` rather than thrown. `syncSettings` and `syncProject` return their failures as `SyncErrorRecord[]` alongside their status fields — a rejected settings PATCH (`syncSettings` still special-cases REST `status === 422` org-policy rejections in the message), an unresolved project, a missing node ID, a failed link, a failed issue listing, or a failed item add — and `syncRepo` folds them into the repository's `errors`, so `success: errors.length === 0` reflects every operation (silk-sync-action#165 closed the settings/project gap). `processRepos` runs repos sequentially through `Effect.partition`; because `syncRepo`'s error channel is `never`, the failure half is statically empty.
